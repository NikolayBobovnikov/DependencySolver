...
Builder::execute(const BuildGraph &build_graph, size_t target_id) const {
  auto top_sorting = _top_sort(build_graph, target_id);

  std::cout << "Topological ordering: " << std::endl;
  for (auto id : top_sorting) {
    std::cout << "Target id " + std::to_string(id) << std::endl;
  }
}
...
Builder::_dfs(const BuildGraph &build_graph, size_t target_id, std::vector<DFS_States> &states,
              std::vector<size_t> &top_sort_ordering) const {
  ...
  top_sort_ordering.push_back(target_id);
}